...
FILE* tok_str_file_inter::tok_file_ = nullptr;
FILE* tok_str_file_inter::sym_file_ = nullptr;

astree* parser::root = nullptr;

void tok_str_file_inter::set_tok_file(string tok_file_name){
   tok_str_file_inter::tok_file_ = fopen(tok_file_name.c_str(),"w");
}

void tok_str_file_inter::set_sym_file(string sym_file_name){
  tok_str_file_inter::sym_file_ = fopen(sym_file_name.c_str(),"w");
}
...
void lexer::include() {
   ...
   if (scan_rc != 2) {
     ...
     fprintf(tok_str_file_inter::tok_file_, "# %2zd \"%s\"\n",
            linenr, filename);
